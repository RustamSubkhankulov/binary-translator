...
4) Push константного значения в стек:
 В данном случае значение, которое будет добавлено в стек, определяется float значением в бинарном коде. Это значение считывается из бинарного кода и добавляется в специализированный массив констант. Затем на этапе патчинга в данную инструкцию подставляется адрес нужного элемента этого массива.
