...
Начало трансляции сопровождается инициализацией инструкций, сохраняющих целочисленные регистрыв стеке в соответствием с соглашением о вызовах System V AMD64 ABI и устанавливающих все XMM регистры в нулевое значение ( инструкции pxor xmm(i), xmm(i) | 0 < i < 15 ). 
...
4) Push константного значения в стек:
 В данном случае значение, которое будет добавлено в стек, определяется float значением в бинарном коде. На основе float значения генерируется unsigned int значение, побайтово равное представлению float числа. Полоученное значение добавляется в стек.
...
Инструкции, вычисляющие математические функции -  POW, SIN, COS и другие, а также IN и  OUT транслируются в вызовы соответсвутющих функций стандартной библеотеки функций, написанной специально для этих инструкций. Функии, вычисляющие синус, косинус и другие математисческие функции, используют библеотеку <math.h>. На первом этапе трансляции смещения в инструкциях call остаются равными нулю, на втором этапе в данные инструкции подставляются высчитанные смещения.
...
- На этапе трансляции в динамическом массиве сохраняется соответствие между всеми адреса во входном бинарном коде и в уже оттранслированном коде.
- На этапе патчинга, происходящем уже после первого этапа, высчитываются необхожимые смещения, используя полученную таблицу соответсвия адресов. Высчитанные смещения подставляются в 'jump'ы и 'call'ы, где это требовалось.
